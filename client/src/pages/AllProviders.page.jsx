...
function AllProvidersPage() {
  ...
  if(!provider.loading && providers && providers.length === 0){
    return(
      <p className='font-semibold w-full text-center py-2.5'>No Tailors Found</p>
    )
  }
  return (
    <>
      {providers && <div>
        ...
        <div className='w-full flex justify-center items-center pb-4'>
          <div className='flex items-center border rounded-full w-96 mx-3 shadow'>
            <div className='flex items-center border bg-white w-full rounded-full overflow-hidden'>
              <input type="text" onKeyDown={handleKeyDown} name="search" value={searchData} onChange={(e)=> setSearchData(e.target.value)} placeholder='Search Tailors Near You' className='w-full h-full px-4 py-2 border-l focus:outline-none flex items-center' id="email"/>
        ...
        <h1 className='font-bold md:text-2xl text-xl  md:px-8 px-2 py-3'>Our Tailors</h1>
        <div className='grid lg:grid-cols-3 sm:grid-cols-2 my-4  gap-6 md:px-8 px-2'>
          ...
          {isSearch && searchData.length>0 && searchedProviders.length === 0 && <p className='font-semibold text-gray-500 text-center w-full py-2.5'>No Tailors Found</p>}
      </div>}
    </>
  )
}
